"""Unit tests for src/fabtwin/spc/xbar_s.py - error handling and detection
behavior, not the constant-derivation/known-answer fixture (that lives in
tests/validation/test_xbar_s_derivation.py).
"""

import pytest

from fabtwin.spc.xbar_s import apply_phase_ii, fit_phase_i
...
def test_s_chart_detects_a_variance_spike_that_xbar_chart_misses():
    """The whole point of the S chart existing alongside Xbar: a subgroup
    whose mean is unchanged but whose spread has blown up should trip the
    S chart even though the Xbar chart sees nothing wrong.
    """
    healthy = [[9, 10, 11, 10, 10]] * 10
    limits = fit_phase_i(healthy)
    same_mean_high_spread = healthy + [[0, 20, 0, 20, 10]]  # mean=10, huge spread
    xbar_points, s_points = apply_phase_ii(same_mean_high_spread, limits)
    assert xbar_points[-1].out_of_control is False
    assert s_points[-1].out_of_control is True
